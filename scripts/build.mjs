...
<div class="hero-name-row"><p class="eyebrow" data-hero="0">${esc(site.hero_label)}</p><p class="hero-location">${esc(site.location)}</p></div>
<div class="hero-layout">
  ...
  <div class="hero-copy">
    ...
    <a class="text-link" href="#ideas" data-hero="5">Explore the questions <span aria-hidden="true">↓</span></a>
  </div>
</div>
...
<section id="ideas" class="section ideas-section" aria-labelledby="ideas-heading"><div class="shell">${sectionHead('ideas','02')}<h2 id="ideas-heading" class="display-heading">What I think about.</h2><div class="ideas-list">${ideas.map((idea,i)=>`<article id="idea-${esc(idea.id)}" class="idea" data-reveal-group><span class="idea-number">0${i+1}</span><div><p class="eyebrow">${esc(idea.category)}</p><h3>${esc(idea.question)}</h3></div><div class="idea-description"><p>${esc(idea.text)}</p>${tags(idea.tags)}<div class="idea-projects">${idea.projects.map(slug=>link(`/research/${slug}/`,`${bySlug[slug].title} ↗`)).join('')}</div></div></article>`).join('')}</div></div></section>
<section id="work" class="section shell" aria-labelledby="work-heading">${sectionHead('work','03')}<div class="section-title-row"><h2 id="work-heading" class="display-heading">Questions into work.</h2><p>Four connected research stories.</p></div><div class="projects">${projects.map(homeProject).join('')}</div></section>
