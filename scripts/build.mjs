...
const [about, beyond, ideas, journey, publications, recognition, methods] = await Promise.all([
  md('about'), md('beyond'), yaml('ideas'), yaml('journey'), yaml('publications'), yaml('recognition'), yaml('methods')
]);
...
function header() {
  return `<a class="skip-link" href="#main">Skip to content</a><header class="site-header"><div class="nav-shell"><a class="wordmark" href="/#home" aria-label="Yizhuo Song home">YS<span class="wordmark-name">Yizhuo Song</span></a><nav class="desktop-nav" aria-label="Primary navigation">${[['ideas','Ideas'],['work','Work'],['journey','Journey'],['contact','Contact']].map(([id,label])=>link(`/#${id}`,label)).join('')}</nav><details class="site-menu"><summary aria-label="All sections"><span>Explore</span><span class="menu-icon" aria-hidden="true">+</span></summary><nav aria-label="All sections">${site.navigation.map(n=>link(`/#${n.id}`,n.label)).join('')}</nav></details></div></header>`;
}
function footer() {
...
function publication(p) {
  return `<article class="publication"><span class="publication-year">${esc(p.year)}</span><div><p class="meta">${esc(p.venue)}</p><h3>${esc(p.title)}</h3><p class="authors">${esc(p.authors)}${p.author_note ? ` <span>${esc(p.author_note)}</span>` : ''}</p><p class="publication-status">${esc(p.status)}</p><div class="output-links">${p.links.map(l=>link(l.url,l.label,'text-link')).join('')}${p.link_placeholder ? `<span class="content-note">${esc(p.link_placeholder)}</span>` : ''}</div>${p.bibtex ? `<details class="citation"><summary>BibTeX</summary><pre><code>${esc(p.bibtex)}</code></pre><button type="button" class="copy-bib" data-copy="bib-${esc(p.id)}">Copy citation</button><textarea id="bib-${esc(p.id)}" class="sr-only" tabindex="-1" aria-hidden="true">${esc(p.bibtex)}</textarea><span class="copy-status" role="status"></span></details>` : ''}</div></article>`;
}
const geneMark = `<svg class="gene-mark" viewBox="0 0 80 80" aria-hidden="true" focusable="false"><g fill="none" stroke-linecap="round" stroke-linejoin="round"><path class="gene-rungs" d="M24 10H56M28 18H52M34 26H46M34 42H46M28 50H52M24 58H56M28 66H52"/><path class="gene-strand" d="M24 6C24 22 56 34 56 54S24 70 24 76"/><path class="gene-strand gene-strand-secondary" d="M56 6C56 22 24 34 24 54S56 70 56 76"/></g><circle cx="56" cy="54" r="3.5" fill="var(--accent)"/></svg>`;
const home = `<main id="main"><section id="home" class="hero shell" aria-labelledby="hero-name">
<div class="hero-name-row"><p class="eyebrow">${esc(site.hero_label)}</p><p class="hero-location">${esc(site.location)}</p></div>
<div class="hero-layout">
  <div class="hero-intro">
    <h1 id="hero-name">${esc(site.name)}<span aria-hidden="true">.</span></h1>
    <p class="hero-affiliation">${esc(site.affiliation)}</p>
    <ul class="identity" aria-label="Research identity">${site.identity.map(i=>`<li>${esc(i)}</li>`).join('')}</ul>
  </div>
  ...
  <div class="hero-copy">
    <p class="hero-statement">${esc(site.statement)}</p>
    <a class="text-link" href="#ideas">Explore the questions <span aria-hidden="true">↓</span></a>
  </div>
</div>
<div class="themes">${site.themes.map((t,i)=>`<div><span class="theme-number">0${i+1}</span><h2>${esc(t.title)}</h2><p>${esc(t.text)}</p></div>`).join('')}</div></section>
<section id="about" class="section shell about" aria-labelledby="about-heading">${sectionHead('about','01')}<div class="about-grid"><h2 id="about-heading" class="display-heading">${esc(about.heading)}</h2><div class="prose">${marked.parse(about.body)}<p class="affiliation">${esc(about.affiliation)}</p></div></div></section>
<section id="ideas" class="section ideas-section" aria-labelledby="ideas-heading"><div class="shell">${sectionHead('ideas','02')}<h2 id="ideas-heading" class="display-heading">What I think about.</h2><div class="ideas-list">${ideas.map((idea,i)=>`<article id="idea-${esc(idea.id)}" class="idea"><span class="idea-number">0${i+1}</span><div><p class="eyebrow">${esc(idea.category)}</p><h3>${esc(idea.question)}</h3></div><div class="idea-description"><p>${esc(idea.text)}</p>${tags(idea.tags)}<div class="idea-projects">${idea.projects.map(slug=>link(`/research/${slug}/`,`${bySlug[slug].title} ↗`)).join('')}</div></div></article>`).join('')}</div></div></section>
<section id="work" class="section shell" aria-labelledby="work-heading">${sectionHead('work','03')}<div class="section-title-row"><h2 id="work-heading" class="display-heading">Questions into work.</h2><p>Four connected research stories.</p></div><div class="projects">${projects.map((p,i)=>`<article class="project project-${i+1}"><div class="project-copy"><p class="eyebrow">${esc(p.category)}</p><h3>${link(`/research/${p.slug}/`,p.title)}</h3><p class="project-headline">${esc(p.headline)}</p><p>${esc(p.summary)}</p>${tags(p.tags)}<a class="text-link" href="/research/${p.slug}/">Read the research story ${arrow}<span class="sr-only">: ${esc(p.title)}</span></a></div><div class="project-side">${diagram(p.diagram,p.headline)}<p class="project-hook">${esc(p.hook)}</p></div></article>`).join('')}</div></section>
<section id="journey" class="section journey-section" aria-labelledby="journey-heading"><div class="shell">${sectionHead('journey','04')}<div class="journey-layout"><div class="journey-intro"><h2 id="journey-heading" class="display-heading">Following the next question.</h2><p>${esc(journey.intro)}</p></div><ol class="journey-chapters">${journey.chapters.map((c,i)=>`<li><span class="chapter-number">0${i+1}</span><div><h3>${esc(c.title)}</h3><p class="context">${esc(c.context)}</p><p>${esc(c.text)}</p></div></li>`).join('')}</ol></div></div></section>
<section id="publications" class="section shell" aria-labelledby="publications-heading">${sectionHead('publications','05')}<h2 id="publications-heading" class="display-heading">Publications &amp; recognition.</h2><div class="publications">${publications.map(publication).join('')}</div><div class="recognition">${recognition.map(r=>`<div><h3>${esc(r.title)}</h3><p>${esc(r.context)}</p></div>`).join('')}</div></section>
<section id="methods" class="section shell methods-section" aria-labelledby="methods-heading">${sectionHead('methods','06')}<h2 id="methods-heading" class="display-heading">Tools for the questions.</h2><div class="methods-grid">${methods.map((m,i)=>`<div><span class="method-index">0${i+1}</span><h3>${esc(m.title)}</h3><ul>${m.methods.map(v=>`<li>${esc(v)}</li>`).join('')}</ul>${m.software?`<p class="software">${esc(m.software)}</p>`:''}</div>`).join('')}</div></section>
<section id="beyond" class="section beyond-section" aria-labelledby="beyond-heading"><div class="shell">${sectionHead('beyond','07')}<div class="about-grid"><h2 id="beyond-heading" class="display-heading">${esc(beyond.heading)}</h2><div class="prose">${marked.parse(beyond.body)}</div></div></div></section>
<section id="contact" class="section shell contact" aria-labelledby="contact-heading">${sectionHead('contact','08')}<div class="contact-grid"><div><h2 id="contact-heading" class="display-heading">Let's talk.</h2><p>${esc(site.contact_intro)}</p><a class="email-link" href="mailto:${esc(site.email)}">${esc(site.email)} ${arrow}</a></div><div class="contact-links"><p class="eyebrow">${esc(site.name)}</p>${site.links.filter(l=>l.label!=='Email').map(l=>l.url?`<a href="${safeURL(l.url)}"><span>${esc(l.label)}${l.detail?`<small>${esc(l.detail)}</small>`:''}</span>${arrow}</a>`:`<div class="unavailable"><span>${esc(l.label)}</span><small>${esc(l.placeholder)}</small></div>`).join('')}</div></div></section></main>`;

await rm(out,{recursive:true,force:true});
await mkdir(out,{recursive:true});
await cp(path.join(root,'public'),out,{recursive:true});
await writeFile(path.join(out,'index.html'),page(`${site.name} — Research`,site.description,home));
