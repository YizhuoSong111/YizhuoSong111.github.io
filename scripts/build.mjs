...
const home = `<main id="main"><section id="home" class="hero shell" aria-labelledby="hero-name"><div class="hero-name-row"><p class="eyebrow">${esc(site.hero_label)}</p><p class="hero-location">${esc(site.location)}</p></div><h1 id="hero-name">${esc(site.name)}<span aria-hidden="true">.</span></h1><ul class="identity" aria-label="Research identity">${site.identity.map(i=>`<li>${esc(i)}</li>`).join('')}</ul><div class="hero-grid"><div class="hero-copy"><p class="hero-statement">${esc(site.statement)}</p><a class="text-link" href="#ideas">Explore the questions <span aria-hidden="true">↓</span></a></div><div class="hero-diagram">${diagram('regulation','A schematic of genetic effects varying across continuous cellular states')}</div></div><div class="themes">${site.themes.map((t,i)=>`<div><span class="theme-number">0${i+1}</span><h2>${esc(t.title)}</h2><p>${esc(t.text)}</p></div>`).join('')}</div></section>
